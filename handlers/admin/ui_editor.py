...
async def ui_editor_main(query: CallbackQuery):
    ...
    text = """🎨 <b>РЕДАКТОР UI</b>
<i>Налаштування інтерфейсу</i>
───────────────
<b>📋 Можливості:</b>
├ Заголовки меню
├ Описи
...
async def ui_edit_roles(query: CallbackQuery):
    ...
    text = """📝 <b>МЕНЮ ЗА РОЛЯМИ</b>
<i>Оберіть для редагування</i>
───────────────
<b>📋 Меню:</b>
├ 🌐 Guest
├ 🌟 Manager
...
async def ui_role_edit(query: CallbackQuery, state: FSMContext):
    ...
    text = f"""{menu_info['icon']} <b>{role.upper()}</b>
───────────────
<b>📋 Поточне:</b>
├ <b>Заголовок:</b> {current_title}
├ <b>Опис:</b> {current_desc[:40]}...
└ <b>Банер:</b> {current_banner[:25]}...
───────────────
<b>💡</b> Оберіть опцію:"""
